...
def _extract_user(ti: TaskInstance):
    ...
    res = requests.get(conn.host + "users?limit=20")
...
with DAG(
    ...
    is_valid_user = BranchPythonOperator(
        task_id="is_valid_user", python_callable=_is_valid_user
    )

    skip_user = EmptyOperator(task_id="skip_user")
